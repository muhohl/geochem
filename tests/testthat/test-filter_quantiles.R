...
make_df <- function() {
  ...
  tibble::tibble(
    ...
    Cu_ppm  = c(rnorm(95, mean = 50, sd = 10), rep(500, 5)),
...
test_that("filter_quantiles replaces upper outliers with NA", {
  ...
  result <- filter_quantiles(df, .cols = c("Cu_ppm", "Zn_ppm"),
                             probs = 0.90, upper_or_lower = "upper")
...
test_that("filter_quantiles preserves non-numeric columns", {
  df     <- make_df()
  result <- filter_quantiles(df, .cols = "Cu_ppm", probs = 0.90)
...
test_that("filter_quantiles preserves original column order", {
  df     <- make_df()
  result <- filter_quantiles(df, .cols = c("Cu_ppm", "Zn_ppm"), probs = 0.90)
...
test_that("filter_quantiles lower tail works", {
  ...
  result <- filter_quantiles(df, .cols = "val", probs = 0.05,
...
test_that("filter_quantiles stops for invalid upper_or_lower argument", {
  df <- make_df()
  expect_error(
    filter_quantiles(df, .cols = "Cu_ppm", upper_or_lower = "middle"),
