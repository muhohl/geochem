...
#> 1    DL-001 0.040293 0.035000 0.054262  2.542
#> 2    DL-002 0.045401 0.026970 0.062833  1.503
#> 3    DL-003 0.087000 0.500000 0.014554  1.806
#> 4    DL-004 0.023043 0.015133 0.214000  1.017
#> 5    DL-005 0.021519 0.110000 0.019958  0.831
#> 6    DL-006 0.048942 0.039040 0.706000  4.201
```
...
`tas_diagram()` returns ggplot2 layers for the Total Alkali Silica
...
whole_rock_data |>
  ...
  dplyr::summarise(median = median(ppm), q25 = quantile(ppm, 0.25),
                   q75 = quantile(ppm, 0.75), .groups = "drop") |>
  ggplot(aes(x = element, group = rock_type, colour = rock_type,
             fill = rock_type)) +
...
| Classification diagrams | `tas_diagram()`, `mgt_class_D_B()`, `geom_dare()` |
